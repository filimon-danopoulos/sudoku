...
export class Slotting implements IStrategy {
  ...
  run(sudoku: Sudoku): boolean {
    ...
    while (initial || changed) {
      ...
      for (
        let blockIndex = 0;
        blockIndex < sudoku.blocks.length;
        blockIndex++
      ) {
        ...
        for (
          let emptyIndex = 0;
          emptyIndex < emptyCellsInBlock.length;
          emptyIndex++
        ) {
          ...
          for (
            ...
            const otherEmptyCellsInBLock = emptyCellsInBlock.filter(
              (c) => c !== emptyCell
            );
            ...
            const isMissingNumberSlotted = otherEmptyCellsInBLock.every((c) =>
              [...c.row.cells, ...c.column.cells].some(
                (c) => c.value === missingNumber
              )
            );
